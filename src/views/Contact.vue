  ...
  <div ref="compContact" class="flex flex-col h-screen bg-primary">
    ...
    >
      <div class="flex mx-auto w-full">
        <div
          class="border-2 h-0 my-auto ml-2 border-light-primary w-4/12 lg:w-5/12"
        ></div>
        <div class="text-accent text-center w-6/12 text-xl lg:w-2/12">
          {{ title }}
        </div>
        <div
          class="border-2 h-0 my-auto mr-2 border-light-primary w-4/12 lg:w-5/12"
        ></div>
      </div>

      <div class="lg:w-8/12 mx-auto mt-16 lg:mt-5">
        <p class="text-dark-white text-3xl lg:text-4xl text-center font-bold">
          {{ subtitle }}
        </p>
      </div>
      ...
      <div class="lg:w-3/12 mx-8 lg:mx-auto mt-16 lg:mt-10">
        <button @click="openEmail()" class="regular-button w-full">
          {{ buttonText }}
        </button>
      </div>
    </div>
...
export default class Contact extends Vue {
  ...
  private message =
    'If you want to make a request, suggest ' +
    'an improvement, report an error or you simply want to say hi, my inbox is always open!';

  public scrollToTop() {
    const rootElement = this.$refs.compContact as HTMLDivElement;
  ...
  public openEmail(): void {
    location.href =
      'mailto:' + this.me.getEmail() + '?subject=Hello!&body=Hi,%20Alex!';
  }
}
</script>